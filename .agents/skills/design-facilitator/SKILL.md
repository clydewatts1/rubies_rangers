...
description: Converts a Stage 2 Brainstorm document into a standardized Stage 3 Technical Design document. Enforces mathematical Moneyball formalisms, Python frozen dataclass contracts, Mermaid diagrams, behavioral walkthroughs, and systematic failure-mode audits.
---

# design-facilitator

## Purpose
`design-facilitator` operates at **Stage 3 (Design)** of the Rubies Rangers engineering lifecycle. It consumes a Stage 2 Brainstorm document (`docs/brainstorm/brn_<ID>_<slug>.md`)—or directly an Issue for Fast-Track items—and generates an authoritative, spec-grade Technical Design document in `docs/design/des_<ID>_<slug>.md`.

This skill integrates the mathematical rigor and institutional trading desk standards of Rubies Rangers:
1. **DAG Lineage & OKF Governance**: Maintains an unbroken provenance chain (`Issue -> Brainstorm -> Design -> Plan -> Tasks -> Implementation -> Playbook`) using standardized Frontloader metadata.
...
- **DAG Lineage Rule**: The Design document MUST link back to the source Brainstorm in its OKF `sources` array: `sources: ["docs/brainstorm/brn_<ID>_<slug>.md"]`.
...
- Note the Origin Issue from the Frontloader to preserve unbroken DAG lineage.
